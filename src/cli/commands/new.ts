...
export async function newCommand(projectName: string | undefined, options: NewCommandOptions): Promise<void> {
    ...
    if (projectExists(slug)) {
        ...
        console.log(chalk.dim(`Use 'ralph edit ${slug}' to modify or 'ralph delete ${slug}' to remove`));
        process.exit(1);
    }
...
function applyOptions(config: ProjectConfig, options: NewCommandOptions): ProjectConfig {
    ...
    // Reviewer
    config.reviewer.enabled = parseBoolean(options.reviewerEnabled, options.noReviewer, config.reviewer.enabled);
    ...
    // Escalation
    config.escalation.enabled = parseBoolean(options.escalationEnabled, options.noEscalation, config.escalation.enabled);
    ...
    if (options.fallbackEnabled || options.fallbackThreshold || options.fallbackSequence) {
        ...
        config.provider_fallback.enabled = parseBoolean(options.fallbackEnabled, options.noFallback, true);
...
function printConfigSummary(slug: string, config: ProjectConfig): void {
    ...
    if (config.escalation.enabled) {
        console.log(`     Enabled:    ${chalk.green('Yes')}`);
        console.log(`     Threshold:  ${chalk.green(`${config.escalation.max_builder_failures} failures`)}`);
    } else {
        console.log(`     Enabled:    ${chalk.yellow('No')}`);
    }
    console.log();
    ...
    console.log(`     Completion: ${chalk.green(config.completion_enabled ? 'Enabled' : 'Disabled')}`);
