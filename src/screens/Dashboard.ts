...
export function loadAllTasks(projectDir: string): Task[] {
    ...
    try {
        const files = fs.readdirSync(specsDir).filter((f: string) => f.startsWith('phase-') && f.endsWith('.json'));
...
export function renderDashboard(projectDir: string): string {
    ...
    if (!summary) {
        ...
        lines.push(theme.muted('  Create .project/specs/tasks/phase-*.json files to enable task tracking.'));
        lines.push('');
    } else {
        // Project info box
        const projectInfo = [
