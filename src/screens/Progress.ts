...
export function renderProgress(projectDir: string): string {
  ...
  lines.push("");
  lines.push(
    `  ${theme.success(summary.completed_tasks.toString())} / ${theme.text(summary.total_tasks.toString())} tasks completed`,
  );
  lines.push("");
  ...
  for (const phaseProgress of summary.phases) {
    const phase = phases.find(
      (p) =>
        `phase-${String(p.phase).padStart(2, "0")}-${p.name.toLowerCase()}` === phaseProgress.id ||
        phaseProgress.id.includes(p.name.toLowerCase()),
    );

    const phasePercent =
      phaseProgress.tasks > 0
        ? Math.round((phaseProgress.completed / phaseProgress.tasks) * 100)
        : 0;

    const statusIcon =
      phaseProgress.completed === phaseProgress.tasks
        ? theme.success("✓")
    ...
    lines.push(
      `  ${statusIcon} ${theme.text(phaseProgress.name.padEnd(20))} ${renderProgressBar(phaseProgress.completed, phaseProgress.tasks, { width: 25 })}`,
    );
  ...
  if (summary.by_provider && Object.keys(summary.by_provider).length > 0) {
    ...
    for (const [provider, count] of Object.entries(summary.by_provider)) {
      ...
      lines.push(
        `  ${providerBadge(provider)} ${renderProgressBar(completed, count, { width: 20 })}`,
      );
    }
    lines.push("");
  }
  ...
  if (summary.by_complexity && Object.keys(summary.by_complexity).length > 0) {
    ...
    for (const [complexity, count] of Object.entries(summary.by_complexity)) {
      ...
      lines.push(
        `  ${complexityBadge(complexity)} ${renderProgressBar(completed, count, { width: 20 })}`,
      );
    }
    lines.push("");
  }
 ...
 */
function countCompletedByComplexity(
  phases: ReturnType<typeof loadPhases>,
  complexity: string,
): number {
  let count = 0;
  for (const phase of phases) {
    count += phase.tasks.filter(
      (t) => t.complexity === complexity && t.status === "completed",
    ).length;
  }
  return count;
}
